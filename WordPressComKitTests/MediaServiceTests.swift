...
class MediaServiceTests: XCTestCase {
    ...
    func testCreateMediaFromImageURL() {
        let siteID = 123
    ...
    func testCreateMediaFromImageInstance() {
        ...
        let payload = UIImagePNGRepresentation(rawImage!)
        XCTAssertNotNil(payload)

        subject.createMedia(payload!, filename: filename, mimeType: mimeType, siteID: siteID) { (media, error) in
            XCTAssertNotNil(media)
            XCTAssertEqual(media?.mediaID, siteID)
            XCTAssertEqual(media?.remoteURL, remoteURL)
            XCTAssertEqual(media?.size, mediaSize)
